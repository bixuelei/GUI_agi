...
class Mywindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def filter_bolts(self):
      ...
      if self.num_bolts==1:
        ...
        xx='['+str(self.positions_bolts[0][0])+', '+str(self.positions_bolts[0][1])+', '+str(self.positions_bolts[0][2])+']'
        self.label_12.setText(xx)
        self.label_12.adjustSize()
        self.label_7.setText("Screws 2:")
        self.label_7.adjustSize()
        xx='['+str(self.positions_bolts[1][0])+', '+str(self.positions_bolts[1][1])+', '+str(self.positions_bolts[1][2])+']'
        self.label_13.setText(xx)
        self.label_13.adjustSize()
        self.label_8.setText("Screws 3:")
        self.label_8.adjustSize()
        xx='['+str(self.positions_bolts[2][0])+', '+str(self.positions_bolts[2][1])+', '+str(self.positions_bolts[2][2])+']'
        ...
        self.label_9.adjustSize()
        xx='['+str(self.positions_bolts[3][0])+', '+str(self.positions_bolts[3][1])+', '+str(self.positions_bolts[3][2])+']'
    ...
    def filter_bolts_but_not_to_show(self):
      if self.search_bolts_again!=self.fileName:
        self.positions_bolts,self.num_bolts,self.bolts=find_bolts(self.motor_points_forecast_in_robot, eps=3, min_points=50)
      self.search_bolts_again=self.fileName
    ...
    def display_gear(self):
      ...
      if self.search_bolts_again!=self.fileName:
        if self.type<=2:
          self.gear,self.posgearaup,self.posgearadown=find_geara(seg_motor=self.motor_points_forecast_in_robot)
        else:
      ...
      if self.num_gear==1:
        ...
        if self.type==2:
          ...
          xx='['+str(self.posgearaup[0])+', '+str(self.posgearaup[1])+', '+str(self.posgearaup[2])+']'
          self.label_12.setText(xx)
          self.label_12.adjustSize()
          self.label_7.setText("TypeA2 lower gear")
          self.label_7.adjustSize()
          self.posgearadown=np.around(self.posgearadown,2)
          xx='['+str(self.posgearadown[0])+', '+str(self.posgearadown[1])+', '+str(self.posgearadown[2])+']'
          self.label_13.setText(xx)
          self.label_13.adjustSize()
        else:
      ...
      if self.cover_existence>0:
        ...
        with open(csv_path, 'a+', newline = '') as f:
            ...
            for i in range(self.num_bolts):
              head = ["screw_"+str(i+1), str(self.positions_bolts[i][0]), str(self.positions_bolts[i][1]), str(self.positions_bolts[i][2]), 
        ...
        with open(csv_path, 'a+', newline = '') as f:
            ...
            csv_writer.writerow(head)
            if self.type<=2:
              head = ["TypeA_upper_gear", str(self.posgearaup[0]), str(self.posgearaup[1]), str(self.posgearaup[2])]
              csv_writer.writerow(head)
              head = ["TypeA_lower_gear", str(self.posgearadown[0]), str(self.posgearadown[1]), str(self.posgearadown[2])]
              csv_writer.writerow(head)
            else:
